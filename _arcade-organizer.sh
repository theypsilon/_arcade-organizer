...
def make_config():
    ...
    config = dict()
    config['MAD_DB'] = ini_parser.get_string('MAD_DB', "https://raw.githubusercontent.com/misteraddons/MiSTer_Arcade_MAD/db/mad_db.json.zip")
...
class Infrastructure:
    ...
    def make_symlink(self, mra_path, name, directory):
        target = Path("%s/%s" % (directory, name))
